...
namespace WebSrv
{
    ...
    static ConfigVar<std::map<std::string, std::map<std::string, std::string>>>::ptr g_mysql_dbs = Configurator::lookup("mysql.dbs", std::map<std::string, std::map<std::string, std::string>>(), "mysql dbs");
                             ...
                             const int &timeout)
    {
        ...
        if (mysql == nullptr)
        {
            SRV_LOG_ERROR(g_logger) << "mysql_init error";
        ...
        {
            SRV_LOG_ERROR(g_logger) << "mysql_real_connect(" << host
                                    << ", " << port << ", " << dbname
    ...
    int MySQL::execute(const char *format, va_list ap)
    {
        ...
        if (r)
        {
            SRV_LOG_ERROR(g_logger) << "cmd=" << cmd()
                                    << ", error: " << getErrStr();
            m_hasError = true;
        }
        else
    ...
    static MYSQL_RES *my_mysql_query(MYSQL *mysql, const char *sql)
    {
        if (mysql == nullptr)
        {
            SRV_LOG_ERROR(g_logger) << "mysql_query mysql is null";
            return nullptr;
        }

        if (sql == nullptr)
        {
            SRV_LOG_ERROR(g_logger) << "mysql_query sql is null";
            return nullptr;
        }

        if (::mysql_query(mysql, sql))
        {
            SRV_LOG_ERROR(g_logger) << "mysql_query(" << sql << ") error:"
                                    << mysql_error(mysql);
            return nullptr;
        }
        ...
        if (res == nullptr)
        {
            SRV_LOG_ERROR(g_logger) << "mysql_store_result() error:"
                                    << mysql_error(mysql);
        }
        return res;
    }
    ...
    MySQLStmt::ptr MySQLStmt::Create(MySQL::ptr db, const std::string &stmt)
    {
        ...
        {
            SRV_LOG_ERROR(g_logger) << "stmt=" << stmt
...
#undef XX
    ...
    int MySQLTransaction::execute(const char *format, va_list ap)
    {
        if (m_isFinished)
        {
            SRV_LOG_ERROR(g_logger) << "transaction is finished, format=" << format;
            return -1;
        }
    ...
    int MySQLTransaction::execute(const std::string &sql)
    {
        if (m_isFinished)
        {
            SRV_LOG_ERROR(g_logger) << "transaction is finished, sql=" << sql;
            return -1;
        }
    ...
    MySQL::ptr MySQLManager::get(const std::string &name)
    {
        ...
        if (it != m_conns.end())
        {
            if (!it->second.empty())
            {
                ...
                {
                    SRV_LOG_WARN(g_logger) << "reconnect " << name << " fail";
                    return nullptr;
                }
            }
        }
    ...
    int MySQLManager::execute(const std::string &name, const char *format, va_list ap)
    {
        ...
        if (!conn)
        {
            SRV_LOG_ERROR(g_logger) << "MySQLManager::execute, get(" << name
                                    << ") fail, format=" << format;
            return -1;
        }
    ...
    ISQLData::ptr MySQLManager::query(const std::string &name, const char *format, va_list ap)
    {
        ...
        if (!conn)
        {
            SRV_LOG_ERROR(g_logger) << "MySQLManager::query, get(" << name
                                    << ") fail, format=" << format;
            return nullptr;
        }
    ...
    MySQLTransaction::ptr MySQLManager::openTransaction(const std::string &name, bool auto_commit)
    {
        ...
        if (!conn)
        {
            SRV_LOG_ERROR(g_logger) << "MySQLManager::openTransaction, get(" << name
                                    << ") fail";
            return nullptr;
        }
    ...
    MySQLManager *WebSrv::MySQLManager::getMySQLManager()
    {
    ...
    {
        auto m = MySQLManager::getMySQLManager()->get(name);
        if (!m)
        {
            return nullptr;
        }
