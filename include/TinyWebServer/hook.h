...
//对系统函数的重新封装，当时设置为hook状态时，将切换到自己封装的函数
namespace WebSrv
{
    /**
     * @brief 查看当前hook状态
 ...
 extern "C"{
    ...
    typedef unsigned int (*usleep_fun)(unsigned int seconds);
    ...
    extern int connectWithTimeout(int fd, const struct sockaddr *addr, socklen_t addrlen, int64_t timeout_ms);
}